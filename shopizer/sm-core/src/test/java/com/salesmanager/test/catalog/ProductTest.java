...
public class ProductTest extends com.salesmanager.test.common.AbstractSalesManagerCoreTestCase {
  ...
  public void testCreateProduct() throws Exception {
    ...
    List<Manufacturer> manufacturers = manufacturerService
        .listByProductsByCategoriesId(store, ids, en);
    ...
    ProductAvailability updatableAvailability = updatableProduct.getAvailabilities().iterator()
        .next();
  ...
  private void testViewImage(Product product) throws Exception {
    ...
    OutputContentFile contentFile = productImageService
        .getProductImage(product.getMerchantStore().getCode(), product.getSku(),
            productImage.getProductImage(), ProductImageSize.SMALL);

    Assert.assertNotNull(contentFile);

    //print small image
    OutputStream outputStream = new FileOutputStream(
        IMAGE_FOLDER + "productImage_small_" + contentFile.getFileName());

    ByteArrayOutputStream baos = contentFile.getFile();
    baos.writeTo(outputStream);

    //get physical original image
    contentFile = productImageService
        .getProductImage(product.getMerchantStore().getCode(), product.getSku(),
            productImage.getProductImage(), ProductImageSize.LARGE);

    Assert.assertNotNull(contentFile);

    //print large image
    outputStream = new FileOutputStream(
        IMAGE_FOLDER + "productImage_large_" + contentFile.getFileName());
