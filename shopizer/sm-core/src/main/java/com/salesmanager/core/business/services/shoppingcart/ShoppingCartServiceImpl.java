...
public class ShoppingCartServiceImpl extends SalesManagerEntityServiceImpl<Long, ShoppingCart>
   ...
   * prices and items based on the actual inventory. This method will remove the shopping cart if no
   * items are attached.
   */
  @Override
  @Transactional
  ...
  /**
   * Get a {@link ShoppingCart} for a given id. Will update the shopping cart prices and items based
   * on the actual inventory. This method will remove the shopping cart if no items are attached.
   */
  @Override
  @Transactional
  ...
  /**
   * Get a {@link ShoppingCart} for a given code. Will update the shopping cart prices and items
   * based on the actual inventory. This method will remove the shopping cart if no items are
   * attached.
   */
  @Override
  @Transactional
...
/*					if (!item.isObsolete()) {
  ...
  private void getPopulatedItem(final ShoppingCartItem item) throws Exception {
    ...
    if ((productAttributes != null && productAttributes.size() > 0) || (cartAttributes != null
        && cartAttributes.size() > 0)) {
  ...
  @Override
  public ShoppingCart mergeShoppingCarts(final ShoppingCart userShoppingModel,
      final ShoppingCart sessionCart,
      final MerchantStore store) throws Exception {
    if (sessionCart.getCustomerId() != null && sessionCart.getCustomerId() == userShoppingModel
        .getCustomerId()) {
    ...
    if (CollectionUtils.isNotEmpty(sessionCart.getLineItems())) {
      Set<ShoppingCartItem> shoppingCartItemsSet = getShoppingCartItems(sessionCart, store,
          userShoppingModel);
  ...
  private Set<ShoppingCartItem> getShoppingCartItems(final ShoppingCart sessionCart,
    ...
    if (CollectionUtils.isNotEmpty(sessionCart.getLineItems())) {
      ...
      for (ShoppingCartItem shoppingCartItem : sessionCart.getLineItems()) {
        ...
        if (product == null) {
          throw new Exception(
              "Item with id " + shoppingCartItem.getProductId() + " does not exist");
        }
        ...
        if (CollectionUtils.isNotEmpty(cartAttributes)) {
          for (ShoppingCartAttributeItem shoppingCartAttributeItem : cartAttributes) {
            ...
            if (productAttribute != null
                && productAttribute.getProduct().getId().longValue() == product.getId()
                .longValue()) {
