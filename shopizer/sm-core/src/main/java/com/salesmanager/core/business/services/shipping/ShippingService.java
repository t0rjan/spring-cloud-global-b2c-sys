...
public interface ShippingService {
  ...
  /**
   * Returns a list of supported countries (ship to country list) configured by merchant when the
   * merchant configured shipping National and has saved a list of ship to country from the list
   */
  ...
  /**
   * ShippingType (NATIONAL, INTERNATIONSL) ShippingBasisType (SHIPPING, BILLING)
   * ShippingPriceOptionType (ALL, LEAST, HIGHEST) Packages Handling
   */
  ...
  /**
   * Provides detailed information on boxes that will be used when getting a ShippingQuote
   */
  List<PackageDetails> getPackagesDetails(List<ShippingProduct> products,
      MerchantStore store) throws ServiceException;

  /**
   * Get a list of ShippingQuote from a configured shipping gateway. The quotes are displayed to the
   * end user so he can pick the ShippingOption he wants
   */
  ...
  /**
   * The {@link ShippingSummary} is built from the ShippingOption the user has selected The
   * ShippingSummary is used for order calculation
   */
  ...
  /**
   * Returns a list of supported countries (ship to country list) configured by merchant If the
   * merchant configured shipping National, then only store country will be in the list If the
   * merchant configured shipping International, then the list of accepted country is returned from
   * the list
   */
   ...
   */
  boolean requiresShipping(List<ShoppingCartItem> items, MerchantStore store)
      throws ServiceException;
