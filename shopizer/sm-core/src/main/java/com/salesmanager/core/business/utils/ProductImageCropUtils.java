...
public class ProductImageCropUtils {
  ...
  private void determineCropArea(int width, int specificationsWidth,
    ...
    double factorWidth =
        new Integer(width).doubleValue() / new Integer(specificationsWidth).doubleValue();
    double factorHeight =
        new Integer(height).doubleValue() / new Integer(specificationsHeight).doubleValue();
...
/*		double factor = 1;
  ...
  public BufferedImage getCroppedImage() throws IOException {
    ...
    Rectangle clip = goal
        .intersection(new Rectangle(originalFile.getWidth(), originalFile.getHeight()));
