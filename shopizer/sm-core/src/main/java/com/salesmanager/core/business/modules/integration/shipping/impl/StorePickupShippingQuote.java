 ...
 *
 * Requires a configuration of a message note to be printed to the client and a price for
 * calculation (should be configured to 0)
 *
...
public class StorePickupShippingQuote implements ShippingQuoteModule,
  ...
  public void prePostProcessShippingQuotes(ShippingQuote quote,
    ...
    Validate.notNull(globalShippingConfiguration,
        "IntegrationConfiguration must not be null for StorePickUp");
    ...
    try {
      ...
      shippingOption.setOptionId(
          new StringBuilder().append(MODULE_CODE).append("_").append(region).toString());

      shippingOption.setOptionPrice(productPriceUtils.getAmount(price));

      shippingOption.setOptionPriceText(productPriceUtils
          .getStoreFormatedAmountWithCurrency(store, productPriceUtils.getAmount(price)));
