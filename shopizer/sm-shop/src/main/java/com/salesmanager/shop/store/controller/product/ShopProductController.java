...
public class ShopProductController {
  ...
  public String displayProductWithReference(@PathVariable final String friendlyUrl,
  ...
  public String displayProduct(@PathVariable final String friendlyUrl, Model model,
  ...
  public String display(final String reference, final String friendlyUrl, Model model,
      HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
    ...
    ReadableProduct productProxy = populator
    ...
    Breadcrumb breadCrumb = breadcrumbsUtils
        .buildProductBreadcrumb(reference, productProxy, store, language, request.getContextPath());
    ...
    if (store.isUseCache()) {
      ...
      //get from the cache
      relatedItemsMap = (Map<Long, List<ReadableProduct>>) cache
          .getFromCache(relatedItemsCacheKey.toString());
    ...
    /** template **/
    StringBuilder template = new StringBuilder().append(ControllerConstants.Tiles.Product.product)
  ...
  public @ResponseBody
  ReadableProductPrice calculatePrice(@RequestParam(value = "attributeIds[]") Long[] attributeIds,
      @PathVariable final Long productId, final HttpServletRequest request,
    ...
    List<ProductAttribute> attributes = productAttributeService
        .getByAttributeIds(store, product, ids);
  ...
  private Attribute createAttribute(ProductAttribute productAttribute, Language language) {
    ...
    List<ProductOptionDescription> descriptions = productAttribute.getProductOption()
  ...
  private List<ReadableProduct> relatedItems(MerchantStore store, Product product,
    ...
    List<ProductRelationship> relatedItems = productRelationshipService
        .getByType(store, product, ProductRelationshipType.RELATED_ITEM);
    if (relatedItems != null && relatedItems.size() > 0) {
      ...
      for (ProductRelationship relationship : relatedItems) {
        ...
        ReadableProduct proxyProduct = populator
            .populate(relatedProduct, new ReadableProduct(), store, language);
