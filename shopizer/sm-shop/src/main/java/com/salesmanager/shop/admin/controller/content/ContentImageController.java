...
/**
 * Manage static content type image - Add images - Remove images
 *
 * @author Carl Samson
 */
@Controller
public class ContentImageController {
  ...
  /**
   * Entry point for the file browser used from the javascript content editor
   */
  @PreAuthorize("hasRole('CONTENT')")
  @RequestMapping(value = {"/admin/content/fileBrowser.html"}, method = RequestMethod.GET)
  public String displayFileBrowser(Model model, HttpServletRequest request,
   ...
   */
  @PreAuthorize("hasRole('CONTENT')")
  @RequestMapping(value = {"/admin/content/contentImages.html",
      "/admin/content/contentManagement.html"}, method = RequestMethod.GET)
  public String getContentImages(Model model, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  ResponseEntity<String> pageImages(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      List<String> imageNames = contentService
          .getContentFilesNames(store.getCode(), FileContentType.IMAGE);
      ...
      if (imageNames != null) {
        ...
        for (String name : imageNames) {
          ...
          entry.put("picture", new StringBuilder().append(request.getContextPath())
              .append(imageUtils.buildStaticImageUtils(store, name)).toString());
  ...
  /**
   * Controller methods which allow Admin to add content images to underlying Infinispan cache.
   *
  ...
  public String displayContentImagesCreate(final Model model, final HttpServletRequest request,
  ...
  /**
   * Method responsible for adding content images to underlying Infinispan cache. It will add given
   * content image(s) for given merchant store in the cache. Following steps will be performed in
   * order to add images
   * <pre>
  ...
  public String saveContentImages(
      @ModelAttribute(value = "contentFiles") @Valid final ContentFiles contentImages,
    ...
    if (CollectionUtils.isNotEmpty(contentImages.getFile())) {
      LOGGER.info("Saving {} content images for merchant {}", contentImages.getFile().size(),
          store.getId());
