...
public class ShippingOptionsController {
  ...
  public String displayShippingOptions(Model model, HttpServletRequest request,
    ...
    if (shippingConfiguration != null) {
      ...
      if (shippingConfiguration.getHandlingFees() != null) {
        shippingConfiguration.setHandlingFeesText(
            priceUtil.getAdminFormatedAmount(store, shippingConfiguration.getHandlingFees()));
      }

      if (shippingConfiguration.getOrderTotalFreeShipping() != null) {
        shippingConfiguration.setOrderTotalFreeShippingText(priceUtil
            .getAdminFormatedAmount(store, shippingConfiguration.getOrderTotalFreeShipping()));
      }
  ...
  public String saveShippingOptions(
    ...
    if (!StringUtils.isBlank(configuration.getOrderTotalFreeShippingText())) {
      try {
        ...
        ObjectError error = new ObjectError("orderTotalFreeShippingText",
    ...
    if (!StringUtils.isBlank(configuration.getHandlingFeesText())) {
      try {
        ...
        ObjectError error = new ObjectError("handlingFeesText",
            messages.getMessage("message.invalid.price", locale));
        result.addError(error);
      }
    }
