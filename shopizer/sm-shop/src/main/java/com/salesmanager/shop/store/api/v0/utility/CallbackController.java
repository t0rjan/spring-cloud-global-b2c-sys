...
/**
 * In progress entry point for receiving various hooks and callback from various web AP.
 *
 * The first draft implementation is for facebook conversations with messenger. The FB messanger API
 * supports the possibility to track all conversations messages which are sent back to the webhook.
 * It accepts request parameters and respond back simple acknowledge answer.
 *
 * A good enhancement would be to have sub-implementation by webhook so this controller could
 * delegate to a worker according to a request parameter. But ... let's start exploring with FB
 * first.
 *
 * @author carlsamson
 */
