...
public class DigitalProductController {
  ...
  public String getDigitalProduct(@RequestParam("id") long productId, Model model,
  ...
  public String saveFile(
      @ModelAttribute(value = "productFiles") @Valid final ProductFiles productFiles,
    ...
    if (CollectionUtils.isNotEmpty(productFiles.getFile())) {
      LOGGER.info("Saving {} product files for merchant {}", productFiles.getFile().size(),
          store.getId());
  ...
  public @ResponseBody
  ResponseEntity<String> removeFile(@RequestParam("fileId") long fileId, HttpServletRequest request,
