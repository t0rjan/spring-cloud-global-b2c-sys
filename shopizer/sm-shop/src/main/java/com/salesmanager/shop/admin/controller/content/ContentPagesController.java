...
public class ContentPagesController {
  ...
  public String listContentPages(Model model, HttpServletRequest request,
  ...
  public String getContentDetails(@RequestParam("id") Long id, Model model,
    ...
    if (!content.getContentType().name().equals(ContentType.PAGE.name())) {
      LOGGER
          .error("This controller does not handle content type " + content.getContentType().name());
      return "redirect:/admin/content/pages/listContent.html";
    }
  ...
  public @ResponseBody
  ResponseEntity<String> removeContent(HttpServletRequest request, HttpServletResponse response,
      Locale locale) {
    ...
    try {
      ...
      if (dbContent != null && dbContent.getMerchantStore().getId().intValue() != store.getId()
          .intValue()) {
        resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
        String returnString = resp.toJSONString();
        return new ResponseEntity<String>(returnString, httpHeaders, HttpStatus.OK);
  ...
  public @ResponseBody
  ResponseEntity<String> pageStaticContent(@RequestParam("contentType") String contentType,
  ...
  public String saveContent(@Valid @ModelAttribute Content content, BindingResult result,
  ...
  /**
   * Check if the content code filled in by the user is unique
   */
  @PreAuthorize("hasRole('CONTENT')")
  @RequestMapping(value = "/admin/content/checkContentCode.html", method = RequestMethod.POST)
  public @ResponseBody
  ResponseEntity<String> checkContentCode(HttpServletRequest request, HttpServletResponse response,
      Locale locale) {
