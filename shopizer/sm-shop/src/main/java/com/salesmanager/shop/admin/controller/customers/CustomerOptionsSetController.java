...
public class CustomerOptionsSetController {
  ...
  public String displayOptions(Model model, HttpServletRequest request,
    ...
    //get values
    List<CustomerOptionValue> optionsValues = customerOptionValueService
        .listByStore(store, language);
  ...
  public String saveOptionSet(@Valid @ModelAttribute("optionSet") CustomerOptionSet optionSet,
    ...
    if (optionSet.getCustomerOption() == null || optionSet.getCustomerOptionValue() == null) {
      model.addAttribute("errorMessageAssociation",
          messages.getMessage("message.optionset.noassociation", locale));
      ObjectError error = new ObjectError("customerOptionValue.id",
          messages.getMessage("message.optionset.noassociation", locale));
    ...
    CustomerOptionValue optionValue = customerOptionValueService
        .getById(optionSet.getCustomerOptionValue().getId());
    ...
    if (optionsSet != null && optionsSet.size() > 0) {
      ...
      for (CustomerOptionSet optSet : optionsSet) {
        ...
        if (opt.getId().longValue() == optionSet.getCustomerOption().getId().longValue()
            ...
            && optValue.getId().longValue() == optionSet.getCustomerOptionValue().getId()
            .longValue()) {
          model.addAttribute("errorMessageAssociation",
              messages.getMessage("message.optionset.optionassociationexists", locale));
          ObjectError error = new ObjectError("customerOptionValue.id",
              messages.getMessage("message.optionset.optionassociationexists", locale));
          result.addError(error);
          break;
    ...
    try {
      ...
      if (optionSet != null && optionSet.size() > 0) {
        ...
        for (CustomerOptionSet optSet : optionSet) {
          ...
          CustomerOptionValueDescription valueDescription = customerOptionValue
  ...
  public @ResponseBody
  ResponseEntity<String> deleteOptionSet(HttpServletRequest request, HttpServletResponse response,
      Locale locale) {
    ...
    try {
      ...
      if (entity == null
          || entity.getCustomerOption().getMerchantStore().getId().intValue() != store.getId()
          .intValue()) {
