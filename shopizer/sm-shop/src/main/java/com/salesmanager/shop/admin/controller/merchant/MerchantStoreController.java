...
public class MerchantStoreController {
  ...
  public String displayStores(Model model, HttpServletRequest request, HttpServletResponse response,
  ...
  public String displayMerchantStoreCreate(Model model, HttpServletRequest request,
  ...
  public String displayMerchantStore(Model model, HttpServletRequest request,
      HttpServletResponse response, Locale locale) throws Exception {
  ...
  public String displayMerchantStore(@ModelAttribute("id") Integer id, Model model,
  ...
  public String saveMerchantStore(@Valid @ModelAttribute("store") MerchantStore store,
    ...
    if ((zonesList == null || zonesList.size() == 0) && StringUtils
        .isBlank(store.getStorestateprovince())) {

      ObjectError error = new ObjectError("zone.code",
          messages.getMessage("merchant.zone.invalid", locale));
      result.addError(error);
    ...
    if (!store.getCode().equals(sessionStore.getCode())) {//create store
      ...
      try {
        ...
        Map<String, String> templateTokens = emailUtils
            .createEmailObjectsMap(request.getContextPath(), store, messages, storeLocale);
        templateTokens.put(EmailConstants.EMAIL_NEW_STORE_TEXT,
            messages.getMessage("email.newstore.text", storeLocale));
        templateTokens.put(EmailConstants.EMAIL_STORE_NAME, messages
            .getMessage("email.newstore.name", new String[]{store.getStorename()}, storeLocale));
