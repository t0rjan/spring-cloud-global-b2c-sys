  ...
  public ReadableProduct populate(Product source,
    ...
    try {
      ...
      for (ProductDescription desc : descriptions) {
        ...
        if (desc.getLanguage() != null && desc.getLanguage().getId().intValue() == language.getId()
            .intValue()) {
          description = desc;
          break;
        }
      ...
      if (source.getManufacturer() != null) {
        ManufacturerDescription manufacturer = source.getManufacturer().getDescriptions().iterator()
            .next();
      ...
      if (images != null && images.size() > 0) {
        ...
        for (ProductImage img : images) {
          ...
          imgPath.append(contextPath).append(
              imageUtils.buildProductImageUtils(store, source.getSku(), img.getProductImage()));
        ...
        if (!CollectionUtils.isEmpty(attributes)) {
          ...
          for (ProductAttribute attribute : attributes) {
            ...
            if (attribute.getAttributeDisplayOnly()) {//read only attribute
              ...
              List<ProductOptionValueDescription> podescriptions = optionValue
              ...
              if (!StringUtils
                  .isBlank(attribute.getProductOptionValue().getProductOptionValueImage())) {
...
/*			Set<Category> categories = source.getCategories();
  ...
  private ReadableProductAttribute createAttribute(ProductAttribute productAttribute,
      Language language) {
