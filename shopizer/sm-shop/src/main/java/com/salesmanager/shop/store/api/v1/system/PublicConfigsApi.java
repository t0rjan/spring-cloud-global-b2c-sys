  ...
  public Configs box(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    try {
      ...
      MerchantConfiguration merchantConfiguration = merchantConfigurationService
          .getMerchantConfiguration(Constants.KEY_FACEBOOK_PAGE_URL, merchantStore);
      if (null != merchantConfiguration) {
        c.setFacebook(merchantConfiguration.getValue());
      }

      merchantConfiguration = merchantConfigurationService
          .getMerchantConfiguration(Constants.KEY_GOOGLE_ANALYTICS_URL, merchantStore);
      if (null != merchantConfiguration) {
        c.setGa(merchantConfiguration.getValue());
      }

      merchantConfiguration = merchantConfigurationService
          .getMerchantConfiguration(Constants.KEY_INSTAGRAM_URL, merchantStore);
      if (null != merchantConfiguration) {
        c.setInstagram(merchantConfiguration.getValue());
      }

      merchantConfiguration = merchantConfigurationService
          .getMerchantConfiguration(Constants.KEY_PINTEREST_PAGE_URL, merchantStore);
      if (null != merchantConfiguration) {
