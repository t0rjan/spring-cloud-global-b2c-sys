...
public class TaxRatesController {
  ...
  public String displayTaxRates(Model model, HttpServletRequest request,
  ...
  public String saveTaxRate(@Valid @ModelAttribute("taxRate") TaxRate taxRate, BindingResult result,
  ...
  public String updateTaxRate(@Valid @ModelAttribute("taxRate") TaxRate taxRate,
  ...
  private void validateTaxRate(Model model, TaxRate taxRate, BindingResult result,
      MerchantStore store, Language language, Locale locale) throws Exception {
    ...
    if (StringUtils.isBlank(taxRate.getRateText())) {
      FieldError error = new FieldError("taxRate", "rateText",
          messages.getMessage("NotEmpty.taxRate.rateText", locale));
    ...
    try {
      ...
      FieldError error = new FieldError("taxRate", "rateText",
          messages.getMessage("message.invalid.rate", locale));
      result.addError(error);
    }
    ...
    if (tr != null) {
      if (tr.getId().longValue() != taxRate.getId().longValue()) {
        FieldError error = new FieldError("taxRate", "code",
            messages.getMessage("NotEmpty.taxRate.unique.code", locale));
        result.addError(error);
      }
    }
  ...
  public String editTaxRate(@ModelAttribute("id") String id, Model model,
