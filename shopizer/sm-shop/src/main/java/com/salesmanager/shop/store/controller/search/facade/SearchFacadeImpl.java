...
public class SearchFacadeImpl implements SearchFacade {
  ...
  /**
   * Index all products from the catalogue Better stop the system, remove ES indexex manually
  ...
  public SearchProductList search(MerchantStore store, Language language,
    ...
    String query = String
        .format(coreConfiguration.getProperty("SEARCH_QUERY"), searchRequest.getQuery());
    SearchResponse response = searchService
        .search(store, language.getCode(), query, searchRequest.getCount(),
            searchRequest.getStart());
    return this
        .copySearchResponse(response, store, searchRequest.getStart(), searchRequest.getCount(),
            language);
  }

  @Override
  public SearchProductList copySearchResponse(SearchResponse searchResponse,
      MerchantStore merchantStore, int start, int count, Language language) throws Exception {
    ...
    if (!CollectionUtils.isEmpty(entries)) {
      ...
      for (Product product : productList.getProducts()) {
        ...
        ReadableProduct p = populator
            .populate(product, new ReadableProduct(), merchantStore, language);
    ...
    if (facets != null) {
      ...
      if (categoriesFacets != null) {
        ...
        List<Category> categories = categoryService
            .listByCodes(merchantStore, categoryCodes, language);
        ...
        for (Category category : categories) {
          ReadableCategory categoryProxy = populator
              .populate(category, new ReadableCategory(), merchantStore, language);
  ...
  public ValueList autocompleteRequest(String query, MerchantStore store, Language language)
    ...
    SearchKeywords keywords = searchService
        .searchForKeywords(req.getCollectionName(), q, AUTOCOMPLETE_ENTRIES_COUNT);
